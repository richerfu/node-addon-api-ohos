...
const beforeItSpecified = function (itDescs, func) {
  return Reflect.has(core, "beforeItSpecified")
    ? core.beforeItSpecified(itDescs, func)
    : (itDescs, func) => {};
};

const afterItSpecified = function (itDescs, func) {
  return Reflect.has(core, "afterItSpecified")
    ? core.afterItSpecified(itDescs, func)
    : (itDescs, func) => {};
};
...
const xdescribe = function (desc, func) {
  return Reflect.has(core, "xdescribe")
    ? core.xdescribe(desc, func, null)
    : (desc, func, reason) => {};
};
xdescribe.reason = (reason) => {
  return (desc, func) => {
    return Reflect.has(core, "xdescribe")
      ? core.xdescribe(desc, func, reason)
      : (desc, func, reason) => {};
  };
};
const xit = function (desc, filter, func) {
  return Reflect.has(core, "xit")
    ? core.xit(desc, filter, func, null)
    : (desc, filter, func, reason) => {};
};
xit.reason = (reason) => {
  return (desc, filter, func) => {
    return Reflect.has(core, "xit")
      ? core.xit(desc, filter, func, reason)
      : (desc, filter, func, reason) => {};
  };
};
