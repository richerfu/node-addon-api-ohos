...
export default function basicTypeValue() {
  describe("basic_type_value_test", () => {
    it("RunValue", DEFAULT, () => {
      const externalValue = binding.basic_types_value.createExternal();

      function isObject(value) {
        return (
          (typeof value === "object" && value !== externalValue) ||
      ...
      function typeCheckerTest(typeChecker, expectedType) {
        ...
        testValueList.forEach((testValue) => {
          if (testValue !== null && expectedType === "object") {
            ...
            assert.strictEqual(
              typeChecker(testValue),
              detailedTypeOf(testValue) === expectedType
            );
          }
        });
      }
        ...
        testValueList.forEach((testValue) => {
          ...
          if (isNaN(expected)) {
            ...
            assert.deepStrictEqual(
              typeConverter(testValue),
              expectedType(testValue)
            );
          }
        });
      }
